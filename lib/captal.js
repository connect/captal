...
$(document).ready(function(){
	...
	readonly	: 0,
        ...
        requestURL      : null,
        remoteURL: {
            game        : 'http://quest-book.ru/online2/getgame/',        
            gamelist    : 'http://quest-book.ru/online2/gamelist',
            statuses    : 'http://quest-book.ru/online2/statuses.json',
            templates   : 'http://quest-book.ru/online2/templates.json',
            publish     : 'http://quest-book.ru/online2/publish/',
            setstatus   : 'http://quest-book.ru/online2/setgamestatus/'
        },
        localURL: {
            ...
            publish     : '../dev/ajax.php?cmd=publish&param=',
    ...
    project.exportRepo = function(){
        if (project.gameID != null) {
            ...
            error('Игра не найдена в репозитории');
        }
    };
...
//debug(page.pageIndex,tpage);
    ...
    repo.game.get = function( gameID ){
        ...
        .success(function(res){                                    
            ...
            if (res.result == 'ok') {              
                ...
                // if not author, and game is published =>
                // => set read only mode & moderator mode 
                if (res.data.IS_AUTHOR == 0) {
                    cfg.readonly = 1;
                    if (res.data.status == 2) {                    
                        $('#mnuProjectModerator').removeClass('hidden');
                    }
                } else {
                    $('#mnuProjectModerator').addClass('hidden');
                    cfg.readonly = 0;
                }
    ...
    repo.game.publish = function( gameID, callback ){
        ...
        $.ajax({
            type: 'POST',
            url: (cfg.debug) ? cfg.localURL.publish + gameID : cfg.remoteURL.publish + gameID,
            dataType: 'json',
            data: xml
        })
        .success(function(res){
            if (res.result == 'ok') {
                ...
                error('Не могу опубликовать игру в репозитории');
            }
        })
        .error(function(res){
            error('Не могу опубликовать игру в репозитории!');
