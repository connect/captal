...
$(document).ready(function(){
     ...
     */
    block.add = function(o) { debug('block.add('+JSON.stringify(o)+')');        
            ...
            .append(
                '<div class="blk_wrapper">\
                    <div class="blk_header">\
                        ...
                        <img class="lang-flag" lang="undefined">\
                    </div>\
                        <div id="'+                 o.uid+'" \
        ...
        editor.on('focus',function(e, ui){
            ...
            $('#nav').jstree(true).select_node( 'nav_'+parID+'_'+o.tag+'_'+blkID , true);
        });
            ...
            $('#'+editorID+' .autolink').each(function(){
                ...
                $(div)
                    ...
                    .click(function(e){
                        ...
                        if (e.which == 1) {                                                        
                            block.removeLinks();
                            paragraph.open( $(this).html(), paragraph.current() );                            
                        }                        
                        return false;
                    })
 ...
 //console.log(blkID,blkTypeN)       
        ...
        tree.create_node( 'nav-'+ o.tag , { 
            type    : o.tag, 
            id      : 'nav_'+ o.paragraph +'_'+ o.tag +'_'+ blkTypeN, 
            text    : (blkTypeN+1) 
        });
    ...
    block.move = function(e){ debug('block.move('+e+')');
        ...
        var tree        = $('#nav').jstree(true);
        var node        = tree.get_node('nav_'+parID+'_'+ tag + '********************' );
    ...
    block.show = function(parID, tag, blkID) { debug('block.show('+parID+', '+tag+', '+blkID+')');
        ...
        if ($('#tab-'+parID).length  == 0) {
            ...
            paragraph.open( parID, parID, function(){
    ...
    block.store = function(editorID){ debug('block.store('+ editorID +')');
        ...
        if (tag == 'handler') {
            //debug( name );
            $('#data-source handlers [name='+ parID +']')
                .html( '&lt;![CDATA[' + source + ']]&gt;' );
        } else {
            
            $('#data-source article#'+ parID +' '+tag +':eq('+index+')')
                .html( '&lt;![CDATA[' + source + ']]&gt;' )
                .attr('if',         logicTRUE)
        ...
        lang.detect(source, function(res){
            ...
            // update flag
            $('#tab-'+parID+' #blk_'+tag+' .blk_header:eq('+blkID+') img.lang-flag:first')
    ...
    block.translate = function(editorID, parID, blkID){ debug('block.translate('+editorID+', '+parID+', '+blkID+')');
        ...
        parID    = parID != undefined ? parID : $('#'+editorID).attr('captal-nav').split('_')[1];
        blkID    = blkID != undefined ? blkID : $('#'+editorID).attr('captal-nav').split('_')[3];
        ...
        $.ajax({
                ...
                success: function(res){
                    ...
                    obj
                        ...
                        .attr('captal-translated', true)
                        .after('<div class="autotrans"><b>Машинный перевод: <img src="img/16/flag_'+tranLang+'.png"></b><div>'+ tranText +'</div>Переведено сервисом <a href="http://translate.yandex.ru/" target="_blank">«Яндекс.Переводчик»</a></div>\
                            <div class="origtrans"><b>Оригинал: <img src="img/16/flag_'+origLang+'.png"></b><div>'+ origText +'</div></div>');
    ...
    paragraph.close = function(obj){ debug('paragraph.close(...)');
    ...
    paragraph.delete = function(parID){ debug('paragraph.delete()');
        ...
        if ($('#data-source article#'+parID).length > 0) {
            ...
            if (confirm('Хотите удалить параграф '+parID+' с его содержимым?')) {            
                var tree = $('#toc').jstree(true);
                var parent = tree.get_node(parID).parent;

                tree.delete_node(parID);
    ...
    paragraph.linkState = function(parID){
        ...
        if ($('#data-source action[goto="'+parID+'"]').length == 0) {
    ...
    paragraph.new = function(parID){ debug('btnNewParagraph.click()');
        ...
        if ($('#data-source').html().length > 0){ 
            ...
            if (tnode.type.indexOf('paragraph') == 0) {
    ...
    paragraph.newInject = function(){ debug('paragraph.newInject()');
        ...
        var parentID = toc.get_node(source).parent;
        var parent   = toc.get_node(parentID);
        var logicGO  = $('#tab-'+source+' #blk_action .blk_wrapper:has(.ace_focus) #logicGO');
    ...
    paragraph.newLinked = function(){
        ...
        if (parID) {
            ...
            if (tag == 'action') {
                ...
                var logicGOicon = $('#tab-'+parID+' .ace_focus').prev().find('#logicGOicon')

                gui.logicGO(logicGOicon);

            } else {

                var obj = $('#tab-'+parID+' #blk_action #logicGO:last');

                // does paragraph contains action block?            
                if ( obj.length > 0 ) {

                    // does last action block have logicGO filled?
                    if ( obj.val().length > 0 ) {

                        // check if that paragraph exists
                        if ( $('#data-source article#'+obj.val() ).length > 0) {
                            
                            // paragraph exists, nothing to do
                            // create new action and linked paragraph for that
                            block.create({
                            ...
                            // paragraph for this action is not exist, create one
                            paragraph.new( obj.val() );
                        }
        ...
        if ( $('#data-source article#'+parID).length == 0 && $('#data-source handler[name="'+parID+'"]').length == 0 ) {
            ...
            if (true) {
                ...
                paragraph.create({
                    ...
                    id: parID,
                    callback: function(node){ 
                        paragraph.open( node.id, curParID, callback );  
                        gui.tocUpdate(); 
                    }
                });*/
                paragraph.new(parID);
    ...
    paragraph.show = function(parID) { debug('paragraph.show('+parID+')');
        ...
        $('#nav').jstree({
            ...
            if ( data.action == 'select_node' ) { 
                ...
                if (data.node.type == 'text' || data.node.type == 'action' || data.node.type == 'script' || data.node.type == 'handler') {
                    ...
                    if( data.node.type == 'action' && keys.state[18] ) { // ALT+Click
                     
                        var target = $('#desk [rile="tabpanel"],[aria-hidden="false"] #blk_action #logicGO:eq('+data.node.id.split('_')[3]+')').val();
                        paragraph.open( target );
                   
                    } else {

                        block.show( paragraph.current(), data.node.id.split('_')[2], data.node.id.split('_')[3] );
                    }
                
                } else if (data.node.type == 'link') {
                    
                    paragraph.open(data.node.text);
                }
            }
        ...
        $('#tab-'+ parID +' .blk_editor').each(function(index){           
            ...
            if (tag == 'script') {
                ...
                name = $(this).attr('captal-type');
                
            } else if (tag == 'action') {
                
                name = '§'+ $(this).attr('captal-goto');
            ...
            tree.create_node( 'nav-'+ tag , { type: tag, id: 'nav_'+ parID +'_'+ tag +'_'+ (counts[tag]-1) , text: name });

            // add link to navigator node
            $(this).attr('captal-nav', 'nav_'+ parID +'_'+ tag +'_'+ (counts[tag]-1));
        });
        ...
        $('#data-source article:has(action[goto="'+parID+'"])').each(function(index){
            ...
            tree.create_node('nav-links', { type: 'link', id: 'nav_link_'+name, text: name });
        });
        ...
        if ( $('#data-source article#'+parID).children().length > 0 ) {
            ...
            // paragraph icon
            tree.set_icon('nav-root', $('#toc').jstree(true).get_icon(parID) );
        }
        ...
        if (state == undefined) {
            ...
            if ($('#tab-'+parID).length > 0) {    
                ...
                if (parID == paragraph.current() ) {
                    ...
                    $('#toolbar .toolbar-sub button.selected').removeClass('selected');
                    $('#toolbar .toolbar-sub button.state-'+state)
                        .addClass('selected')
                        .trigger('blur');
                }
            }
...
//$('#data-source').html(res);return;
    ...
    project.loadXML = function(res, callback) { debug('project.LoadXML(...)');                
        ...
        if (book.length == 0){
            ...
            $('#btnUseWrapMode').prop(  'disabled', false);
            ...
            $('#toc').jstree({
                    ...
                    check_callback : function(e, node, node_parent, node_position, more){                      
                        if (e == 'move_node') {
                            ...
                            if (node_parent.type.indexOf('folder') == -1) {
                                ...
                                if (more.pos != 'i' && more.ref.type.indexOf('paragraph') == -1) {
                ...
                dnd: { 
                    ...
                    is_draggable: function(node){
                        ...
                        if (node[0].type.indexOf('paragraph') == -1) {
                            return false;
                        } else {
                            return true;
                        }
                ...
                if ( data.action == 'select_node' && (data.node.type.indexOf('paragraph') == 0 || data.node.type == 'handler') ) {  
                ...
                */    
                ...
                if (data.node.id == 'toc-book') {
                ...
                } else if (data.node.type.indexOf('paragraph') == 0){
                    // rename paragraph
                    if ( $('#data-source article#'+data.text).length == 0 ) {
                        ...
                        .each(function(index, value) {
                            ...
                            if (node.id.indexOf('nav_'+data.old+'_') == 0) {
                                
                                var newID = node.id.replace('nav_'+data.old,'nav_'+data.text);
...
//debug(page.pageIndex,tpage);
    ...
    project.preloadTXT = function(raw){ debug('project.preloadTXT()'); 
        ...
        for (var i in res) {
            ...
            var line = $.trim(self); // possible issues of loosing space
            var num  = line.match(/\d+/); // does line conatins numbers?                                            
            ...
            if (cParTags && num != null) {   
                ...
                if ( self.indexOf(iParLeft) == 0 ) {                    
                    ...
                    // if line has paragraphOpenMarker + number + paragraphCloseMarker
            ...
            while (pos != -1) {
                ...
                if (pos == -1) { // last   
                    ...
                    var div = $('<div/>')
                        ...
                        /*
                        .html('<div>\
                                <img src="img/16/icon-'+tag+'.png">\
                                </div>\
                                <div class="par-id">'+parID+'</div>\
                                <div class="par-content" onclick=block.show("'+parID+'","'+tag+'",'+blkID+')>'+ $(this)*/                                        
                        .html('<div>\
    ...
    gui.contextMenuToc = function(node) {
        ...
        } else if ( node.type.indexOf('paragraph') == 0 ) {
    ...
    gui.logicFieldUpdate = function(e, doStore){ //debug('gui.logicFieldUpdate(..)');
        ...
        doStore      = doStore || true;
        
        debug('gui.logicFieldUpdate('+editorID+')') ;
        
        // update navigator with action target
        if ( $(e).attr('id') == 'logicGO') {
            ...
            if (nav) {
                ...
                nav.set_text( 'nav_'+parID+'_action_'+blkID  , '§'+ $(e).val() );
            }
            
        } else if ( $(e).attr('id') == 'logicTYPE') {
            ...
            if (nav) {
                ...
                nav.set_text( 'nav_'+parID+'_script_'+blkID  , $(e).val() );
            }
        }
    ...
    gui.logicGO = function(e){ debug('gui.logicGO()');
        ...
        if (parID == '') {
            ...
            paragraph.open( parID, paragraph.current() );
        ...
        $('#dlgSearch').dialog({
            ...
            buttons: [{
                ...
                click: function(){  
                    ...
                    var index       = $('#dlgSearch #search-results .highlight').index( current );
                    ...
                    while (pos != -1) {                                                
                        ...
                        if (i == index) {
                            ...
                            project.searchNext(index); // start searching from index
                            return;
                        }              
    ...
    gui.setLang = function(e){
        ...
        if (
                (tLang == 'sourceLang' && project.targetLang == langValue) ||
                (tLang == 'targetLang' && project.sourceLang == langValue)
            ) {
