...
$(document).ready(function(){
     ...
     */
    lang.detectOLD = function(str, callback){ debug('lang.detect(...)');
...
(function(){
    ...
    this.findLink = function(row, column) {
        ...
        var match = this.getMatchAround(/[\#№][^\s]+/g, line, column);
        if (!match)
            return;
...
editor.hoverLink.on("open", function(e) {
   console.log("trying to open",e.value.substr(1))
}) */
        }    
        ...
        $(editor.textInput.getElement()).on('keydown',function(e, ui){
            ...
            var parID    = paragraph.current(),
                ...
                actCount = (body.match(/\#\d+/g) || []).length;
            ...
            if (e.key == '#' || e.key == '№' ){
                ...
                if ((body.lastIndexOf('#') == body.length-1 || body.lastIndexOf('№') == body.length-1 ) && body.length > 0) {
                              
                    var newID = paragraph.newID();

                    body = body.substr(0, body.length-1) +'#'+ newID;
            ...
            } else if ([32].indexOf(e.keyCode) >= 0 || ['!','?',':',',','.'].indexOf(e.key) >= 0) {
            ...
            // check for intext action removal
            if (actCount < $('#data-source article#'+parID+' action[intext]').length) {
                
                //console.log('some intext action was removed')
                
                $('#data-source article#'+parID+' action').each(function(index){
                    
                    if ( $(this).attr('intext') > 0 ) {
                    
                        if (body.match( new RegExp('\#'+ $(this).attr('goto')+'\b','g' )) == null ) {
        ...
        // spellcheck
        if ((o.tag == 'text' || o.tag == 'action') && o.body != '') {
    ...
    block.create = function(o, callback){ debug('block.create('+o.tag+')');
        ...
        if (o.tag == 'handler') {
            ...
            if (o.intext)      { obj.attr('intext', o.logicGO);      }
        }    
        ...
        $.ajax({
            ...
            success: function(res){
                ...
                for (var i in res){
                    ...
                    editor.session.addMarker(new Range(pos.start.row, pos.start.column, pos.end.row, pos.end.column),"ace_spellcheck");                    
                }                 
