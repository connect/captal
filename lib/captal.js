...
$(document).ready(function(){
    ...
    $('.version').html("0.17 alpha");
    
    cfg = {
        debug           : 0,
        showInvisibles  : 0,
        showWelcome     : 0,
        gameURL         : 'http://quest-book.ru/online2/getgame/',        
        gamelistURL     : 'http://quest-book.ru/online2/gamelist',
        statusesURL     : 'http://quest-book.ru/online2/statuses.json',
        //gameURL         : '../dev/ajax.php?cmd=getgame&',
        //gamelistURL     : '../dev/ajax.php?cmd=gamelist&',
        //statusesURL     : '../dev/ajax.php?cmd=statuses.json',
        requestURL      : null
    };    
            ...
            while( pos != -1){
                ...
                if ( pos > 0 ) {
                    ...
                    if ( (prevCh < 48 || prevCh > 57 || isNaN(prevCh)) && (nextCh < 48 || nextCh > 57 || isNaN(nextCh) ) ) {
                        ...
                        if (prevCh == 32) {                            
                            opening = left[ text.length-2 ];
                        } else {
                            // else, get everthing up to space
                            opening = left[ text.length-1 ];
                        }
                        
                        // cut right side
                        right = text.substring(pos + num.toString().length);
    ...
    //getMarks("Однажды в студеную зимнюю пору\n я из лесу вышел 321 \n и снова зашел 123.");
    
    /**
     * Analyze text, find numbers and return theirs markers with frequency
     * @param {type} str
     * @returns {undefined}
     * @TODO: write me!
     */
    huntMarkers = function(lines, trails){
     ...
     */
    block.add = function(o) { debug('block.add(',o.name,o.tag,o.lang,')');        
        o.uid  = salt();
        o.mode = '';   
        o.body = o.body || '';  
        ...
        if (o.tag == 'handler') {
            $('#desk #tab-'+ o.id).append(
                '<div class="blk_header" onclick="block.headerClick(this)">'+o.name+':</div>\
                <div id="'+                 o.uid+'" \
            ...
            if (o.lang == 'undefined') {
                // block has no language specified, detect and update
                //lang.detect( o.body , function(stats){
                    //o.lang = stats[0].name;                    
                                        
                    //$('#data-source article#'+ o.id +' '++' img.lang-flag').attr('src', 'src="img/16/flag_'+o.+'.png"')
                    //$('#desk #tab-'+ o.id +' '+ o.tag +'[type="'+ o.name +'"] img.lang-flag').attr('src', 'src="img/16/flag_'+ o.lang +'.png"')                    
                    //'<img class="lang-flag" ><div class="blk_header" onclick="block.headerClick(this)"></span>'+o.name+':</div>'
                    //$('#data-source folder article#'+ o.paragraph +' '+ o.tag +'[type="'+ o.name  +'"]' ).attr('lang', o.lang);
                    /*
                    $('#data-source article#'+ o.paragraph +' '+ o.tag +'[type="'+ o.name  +'"]' )
                        .attr('lang', lang.detect(o.body, function(){
                            $('#data-source article#'+ o.paragraph +' '+ o.tag +'[type="'+ o.name  +'"]' ).attr()
                        }));

                    $('#data-source article#'+ o.paragraph +' '+ o.tag +'[type="'+ o.name  +'"]' ).attr('lang', o.lang);
                    */
                //});                
            }      
            
            $('#desk #tab-'+ o.id +' #blk_'+o.tag).append(
                '<img class="lang-flag" src="img/16/flag_'+o.lang+'.png"><div class="blk_header" onclick="block.headerClick(this)"></span>'+o.name+':</div>\
                <div id="'+                 o.uid+'" \
                    captal-paragraph="'+    o.paragraph+'" \
                    captal-name="'+         o.name+'" \
                    captal-tag="'+          o.tag+'" \
                    captal-if="'+           o.logicTRUE+'" \
                    captal-ifnot="'+        o.logicFALSE+'" \
                    captal-goto="'+         o.logicGO+'" \
                    captal-do="'+           o.logicDO+'" \
                    captal-isglobal="'+     o.logicGLOBAL+'" \
                    class="blk_editor">'+ o.body +'</div>' );
        }
        $('#desk').tabs("refresh");
        ...
        editor.on('focus',function(e, ui){                  
            ...
            //debug('focus gained: '+id);            

            if ($('#tab-'+ paragraph.id+ ' #'+ block.id).length > 0) {
                // we already have some block selected, deselect
            ...
            $('#tab-'+ paragraph.id +' .ace_focus').not('#'+ block.id).removeClass('ace_focus');
            $('#'+ block.id).addClass('ace_focus');

            // filter logic fields according to tag
            switch (o.tag) {
                case 'text':
                        $('#logicTRUE').prop(   'disabled', false);
                        $('#logicFALSE').prop(  'disabled', false);
                        $('#logicDO').prop(     'disabled', true);
                        $('#logicGO').prop(     'disabled', true);
                        $('#logicGLOBAL').prop( 'disabled', true);
                    break;
                case 'action':
                        $('#logicTRUE').prop(   'disabled', false);
                        $('#logicFALSE').prop(  'disabled', false);
                        $('#logicDO').prop(     'disabled', false);
                        $('#logicGO').prop(     'disabled', false);
                        $('#logicGLOBAL').prop( 'disabled', true);
                    break;
                case 'script':
                case 'handler':
                        $('#logicTRUE').prop(   'disabled', true);
                        $('#logicFALSE').prop(  'disabled', true);
                        $('#logicDO').prop(     'disabled', true);
                        $('#logicGO').prop(     'disabled', true);
                        $('#logicGLOBAL').prop( 'disabled', false);
                    break;
            }                            
    ...
    block.create = function(o){ debug('block.create('+o.tag+')');
        //var name = $('#tab-'+ paragraph.id +' .blk_header').length +1;       
        o = (typeof o != 'object') ? { tag: o } : o;
        o.name = o.name || getRandomName(o.tag);
        o.body        = o.body        || '',
        o.logicGO     = o.logicGO     || '',
        o.logicDO     = o.logicDO     || '',
        ...
        if (o.tag == 'handler') {
            // if handler
            //debug(paragraph.id);
            $('#data-source handlers').append('<'+o.tag+' name="'+paragraph.id+'" type="'+o.name+'"></'+o.tag+'>');
        } else {
            // if article
            $('#data-source article#'+ paragraph.id).append('<'+o.tag+' goto="'+o.logicGO+'" type="'+o.name+'" if="'+o.logicTRUE+'" ifnot="'+o.logicFALSE+'" do="'+o.logicDO+'">&lt;![CDATA['+o.body+']]&gt;</'+o.tag+'>');            
        }                      
        ...
        tree.create_node( 'nav-'+ o.tag , { type: o.tag, id: 'nav_'+o.name, text: o.name });
    ...
    block.delete = function(e){
        ...
        if (confirm('Удалить блок '+node.text+'?')){        
            ...
            if ( node.type == 'handler') {
                // handler
                $('#data-source handler[type='+node.text+']').remove();
            } else {
                // article
                $('#data-source article#'+paragraph.id+' *[type='+node.text+']').remove();                            
            }                        
            ...
            if ( $('#data-source article#'+paragraph.id).html().trim() == '' ) {
                //$('#data-source article#'+paragraph.id).append('<text type="'+getRandomName('text')+'"></text>');
                block.create('text');
    ...
    /* function(obj) {
        ...
        if (confirm('Удалить блок?') ) {
            var res     = $(obj).parents('a');
            var id      = res.attr('href');
            var tag     = res.attr('captal-tag');
            var type    = res.attr('captal-type');
            $(obj).parents('li').remove();
            $(id).remove();
            $('#data-source article#'+ paragraph.id +' '+tag+'[type="'+ type +'"]').remove();
            $("#desk").tabs("refresh");

            // if it was last block create text
            if ( $('#data-source article#'+paragraph.id).html().trim() == '' ) {
                $('#data-source article#'+paragraph.id).append('<text type="1"></text>');
                paragraph.open(paragraph.id);
            }            
        }
    ...
    block.headerClick = function(e) {
        var obj = $(e).html();
        block.show( obj.substring( 0, obj.length - 1 ) );
    };
    ...
    block.load = function(blockID){ debug('block.load('+blockID+')');
        ...
        $('#desk .field input[type="text"]').val(''); // clear logic fields

        var obj         = $('#tab-'+ paragraph.id +' #'+ blockID);                
        var tag         = $(obj).attr('captal-tag')     || '';
        var name        = $(obj).attr('captal-name')    || '';
        ...
        $('#nav').jstree(true).select_node ('nav_'+ $('#'+blockID).attr('captal-name'), true );
    ...
    block.move = function(e){ debug('block.move('+e+')');
        ...
        var tree        = $('#nav').jstree(true);
        var node        = tree.get_node('nav_'+$(blk).attr('captal-name'));
        var source      = $('#data-source '+node.type+'[type="'+node.text+'"]');
    ...
    block.rename = function(e){                        
        var oldName = $('#tab-'+paragraph.id+' #'+block.id).attr('captal-name');
        ...
        if (newName != null) {
            ...
            if (tag == 'handler') {      
                ...
                $('#data-source handler[name='+paragraph.id+'][type="'+ oldName +'"]').attr('type', newName);
            } else {
                $('#data-source article#'+paragraph.id+' '+tag+'[type="'+ oldName +'"]').attr('type', newName);
            }
        }
    };
     ...
     * @returns {undefined}
     */
    block.show = function(ID) { debug('block.show('+ID+')');
        var uid    = $('#tab-'+ paragraph.id +' .blk_editor[captal-name="'+ID+'"]').attr('id');
        var offset = $('#'+uid).prev().position().top;       
        
        if (offset != undefined) {            
            $('#tab-'+ paragraph.id).scrollTop( $('#tab-'+paragraph.id).scrollTop() + offset );
            ace.edit( uid ).focus();
        }                
    };
    ...
    block.store = function(o){ //debug('block.store('+blockID+')');
        ...
        if (tag == 'handler') {
            //debug( name );
            $('#data-source handlers [name='+ paragraph.id +'][type='+name+']')
                .html( '&lt;![CDATA[' + source + ']]&gt;' );
        } else {
            $('#data-source article#'+ paragraph.id +' '+tag +'[type="'+name+'"]')
                .html( '&lt;![CDATA[' + source + ']]&gt;' )
                .attr('if',         logicTRUE)
        ...
        lang.detect(source, function(res){
            ...
            $('#desk #'+blockID).prevAll('img.lang-flag:first').prop('src', 'img/16/flag_'+tlang+'.png');
            if (tag != 'handler') {
                $('#data-source article#'+paragraph.id+' '+ tag +'[type="'+ name +'"]').attr('lang',tlang);
            }
        })
    };   
    ...
    lang = {  
        ...
        rus: ['артефакт','без','большой','бы','быть','в','вам','вас','ваш','вверх','вернитесь','весь','вниз','во',
            ...
            'они','от','очень','первый','переходите','по','под','показать','после','потом','при','применить','проверьте',
        ...
        eng: ['the','be','of','and','in','to','have','it','for','I','that','you','he','on','with','do','at','by',
            ...
            'very','when','may','way','look','like','use','such','how','because','good','find','man','our','want',
    ...
    paragraph.close = function(obj){ debug('paragraph.close('+ obj +')');
    ...
    paragraph.create = function(o){ debug('paragraph.create('+ JSON.stringify(o) +')');
        if ($('#data-source').html().length > 0) {
            ...
            tree.create_node( o.parent.id , { 
                ...
                // update data-source
                $('#data-source folder'+parentDOM).append('<article id="'+newID+'"><text type="'+getRandomName('text')+'"></text></article>');
    ...
    paragraph.delete = function(e){ debug('paragraph.delete()');
        if (paragraph.id) {
    ...
    paragraph.new = function(){
        debug('btnNewParagraph.click()');
    ...
    paragraph.open = function(parID){        
        debug('paragraph.open('+parID+')');
        ...
        if ( $('#data-source article#'+parID).length == 0 && $('#data-source handler[name="'+parID+'"]').length == 0 ) {
            ...
            if (confirm('Этот параграф не существует, создать?')){            
                ...
                // check if link exist
                if ($('#data-source article#'+paragraph.id+' action[goto="'+parID+'"]').length == 0) {                    
            ...
            if (max > 0) {
                ...
                $('#data-source article#'+parID).children().each(function(){
                    ...
                    if ( $(this)[0].localName == 'metadata') { return true; } // skip metadata block from appearing                       
                    ...
                    block.add({
                        ...
                        body        : $(this)[0].innerHTML
                                                ...
                                                .replace(']]>','')
                                                .replace(']]&gt;',''),
                        name        : $(this).attr('type')      || randomName,
    ...
    paragraph.show = function(parID) { debug('paragraph.show('+parID+')');
        ...
        $('#nav').jstree({
            ...
            contextmenu: { items: gui.contextMenuNav },
            types: {
            ...
            if (newName != null) {
                ...
                if (data.node.type == 'handler') {
                    $('#data-source handler[type="'+ oldName +'"]').attr('type', newName);
                } else {
                    $('#data-source article#'+paragraph.id+' '+tag+'[type="'+ oldName +'"]').attr('type', newName);
                }
            }
        ...
        $('#tab-'+ parID +' .blk_editor').each(function(){
            ...
            var nav = tree.create_node( 'nav-'+ tag , { type: tag, id: 'nav_'+name, text: name });
    ...
    paragraph.shuffle = function(){ 
        debug('paragraph.shuffle()');
    ...
    project.exportDOCX = function(){                                                                                                                                      
        ...
        $('#data-source article').each(function(){
            ...
            doc.append('<h2>'+par+'<a name="'+par+'"></a></h2>');
    ...
    project.importDOCX = function(f){ debug('project.importDOCX()');
        ...
        var jf = new JsFile(f, { workerPath: 'lib/workers/'});
            ...
            open        : function(){
                ...
                repo.game.list(function(res){
                    ...
                    if (res.result == 'ok') {
                        ...
                        // error
                        $('#dlgRepo #list').append('<div class="message">Репозиторий не доступен</div>');
                    }
                });
            },
            close: function() {
...
//$('#data-source').html(res);return;
     ...
     * @returns {undefined}
     */
    project.loadXML = function(res, callback) { 
        debug('project.LoadXML(...)');                
        ...
        if (book.length == 0){
            ...
            $('#toc').jstree({
                ...
                contextmenu: { items: gui.contextMenuToc },
                types: {
                ...
                $('#data-source book handlers').before('<article id="'+n+'"><text type="'+getRandomName('text')+'"></text></article>');
                ...
                */
                ...
                if (data.node.id == 'toc-book') {
                    // rename book
                    $('#data-source book').attr('title',data.text);
                ...
                if ($('#data-source folder').length == 0) {
                    ...
                    if (last > 0) {                        
                        ...
                        $('#data-source book').append('<folder id="folder_0000" type="autofolder"><article id="0"><text type="text_0000"></text></article></folder>');
    ...
    project.new = function(){
        ...
        $('#new-project-template book').attr('guid',salt);
    ...
    project.preloadDOCXalt = function(doc){ debug('project.preloadDOCX()');
        ...
        $('#dlgImportDOCX .pnlBlock.paragraph').append('<div><input id="cParAlone"  type="checkbox" class="paragraph"> На всю строку</div>');        
        ...
        $(doc).find('p').each(function(){
            ...
            if ( tclass != undefined && tclass != '' && classes.indexOf(tclass) < 0) {
                if (classes.length == 0) {
                    $('#dlgImportDOCX .pnlBlock.paragraph').append('<input id="cParClass"  type="checkbox" class="paragraph"> Стиль: <select id="sParClass"><option value="'+tclass+'">'+tclass+'</option></select>');
                } else {
            ...
            if (!centered) {
                if ( p.css('text-align') == 'center' ) {
                    centered = true;
                    $('#dlgImportDOCX .pnlBlock.paragraph').append('<div><input id="cParCenter" type="checkbox" class="paragraph"> По центру</div>');
                }
            };                         
            ...
            $(p).find('span').each(function(){
                ...
                if (!bold) {
                    if ( span.css('font-weight') == 'bold' ) {
                        bold = true;
                        $('#dlgImportDOCX .pnlBlock.paragraph').append('<div><input id="cParBold"   type="checkbox" class="paragraph"> Полужирный</div>');
                    }
                };
                ...
                if (!linked) {
                    if ( span.hasClass('Hyperlink') ) {
                        linked = true;
                        $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLink"   type="checkbox" class="action"> Гиперссылка</div>');
                    }
                };  
            ...
            if (i == last ) {                
                /*
                if (act_to == 10) { $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkTo"   type="checkbox" class="action"> Действия: то xxx </div>'); }                
                if (act_mi == 10) { $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkMi"   type="checkbox" class="action"> Действия: - xxx </div>'); }                
                if (act_rb == 10) { $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkRb"   type="checkbox" class="action"> Действия: (xxx) </div>'); }
                if (act_qm == 10) { $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkQM"   type="checkbox" class="action"> Действия: ?...xxx </div>'); }
                */
    ...
    project.preloadDOCXnew = function(doc){ debug('project.preloadDOCX()');
        ...
        $(doc).find('p').each(function(){
            ...
            if (i == last){
                ...
                for (var tn in acts){
                    if (sums[tn] > 9)
                    $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkU'+tn+'"   type="checkbox" class="action"><b>'+acts[tn]+'</b> xxx (<i>'+sums[tn]+' раз</i>)</div>');
                }
        ...
        $(doc).find('p').each(function(){
            ...
            if (!centered) {
                if ( p.css('text-align') == 'center' ) {
                    centered = true;
                    $('#dlgImportDOCX .pnlBlock.paragraph').append('<div><input id="cParCenter" type="checkbox" class="paragraph"> По центру</div>');
                    $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActCenter" type="checkbox" class="action"> По центру</div>');
                }
            };                         
            ...
            $(p).find('span').each(function(){
                ...
                if (!bold) {
                    if ( span.css('font-weight') == 'bold' ) {
                        bold = true;
                        $('#dlgImportDOCX .pnlBlock.paragraph').append('<div><input id="cParBold"   type="checkbox" class="paragraph"> Полужирный</div>');
                        $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActBold"   type="checkbox" class="action"> Полужирный</div>');
                    }
                };
            ...
            if (i == last ) {                
                ...
                if (act_to > 5) { $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkTo"   type="checkbox" class="action"><b>то</b> xxx (<i>'+act_to+' раз</i>)</div>'); }                
                if (act_mi > 5) { $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkMi"   type="checkbox" class="action"><b>-</b> xxx (<i>'+act_mi+' раз</i>)</div>'); }                
                if (act_rb > 5) { $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkRb"   type="checkbox" class="action"><b>(</b>xxx<b>)</b> (<i>'+act_rb+' раз</i>)</div>'); }
                if (act_qm > 5) { $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkQM"   type="checkbox" class="action"><b>?...</b>xxx (<i>'+act_qm+' раз</i>)</div>'); }
                if (act_ph > 5) { $('#dlgImportDOCX .pnlBlock.action').append('<div><input id="cActLinkPh"   type="checkbox" class="action"><b>параграф</b> xxx (<i>'+act_ph+' раз</i>)</div>'); }
...
//debug(page.pageIndex,tpage);
    ...
    project.preloadTXT = function(raw){ debug('project.preloadTXT()');                             
        ...
        for (var tn in acts) {  
            // treshold is 2
            if (sums[tn] > 2) { 
                $('#dlgImportTXT #pnlActOptions').append('<div class="action-div"><input id="cActLinkU'+tn+'"   type="checkbox" class="action"><b>'+acts[tn]+'</b> xxx (<i>'+sums[tn]+'</i> раз)</div>');
            }
        }      
...
//debug(f.type); return;
    ...
    repo.game.approve = function( gameID ){
        
    };
    
    repo.game.ban = function( gameID ){
    ...
    repo.game.get = function( gameID ){
        ...
        repo.gameURL = (cfg.debug) ? 'lib/repo_game_1.json' : cfg.gameURL + gameID;
        
        $.get(repo.gameURL,function(res){
            if (typeof res == 'string') {
                res = $.parseJSON(res);
            }
            
            if (res.result == 'ok') {                                
                var xml = $.parseXML(res.data.data);
                project.loadXML( xml );     
                ...
                error('Не могу открыть игру из репозитория');
            }
        }).error(function(){
            error('Не могу открыть игру из репозитория');
        });                
    ...
    repo.game.list = function( callback ){
        ...
        repo.gamelistURL = (cfg.debug) ? 'lib/repo_gamelist.json' : cfg.gamelistURL;

        $.get(repo.gamelistURL, function( res ){
            if (typeof res == 'string') {
                res = $.parseJSON(res);
            }
            ...
            if (res.result == 'ok') {                
                ...
                error('Не могу открыть репозиторий');
            }                                     
        }).error(function(){
            if (typeof callback === 'function'){
                callback(false);
    ...
    repo.game.publish = function( gameID ){
        
    };
    
    repo.game.reject = function( gameID ){
        
    };
    
    repo.game.status = function( gameID, status ){
        
    };
    
    repo.game.update = function( gameID ){
    ...
    repo.statusesURL = (cfg.debug) ? 'lib/repo_statuses.json' : cfg.statusesURL;
    $.get(repo.statusesURL,function(res){
        if (typeof res === 'string') {
            res = $.parseJSON(res);
        }
        repo.statuses = res;
    ...
    gui.createFolder = function(){
        if ($('#data-source').html().length > 0) {
            ...
            tree.create_node('toc-paragraphs',{
                ...
                // update data-source
                $('#data-source folder:last').before('<folder id="'+node.id+'" name="'+node.text+'" type="folder_yellow"></folder>');
            });
        } else {
            error('Проект не загружен');
        }
    ...
    gui.scriptAutocomplete = function(request, response){                
        var res = [];
        var i = 0;
        var last = $('#data-source script[type*="'+request.term+'"]').length;

        $('#data-source script[type*="'+request.term+'"]').each(function(){
            i++; 
            res.push( $(this).attr('type') );                    
            if (i == last){
                response(res);
            }
        });                
                    ...
                    mnuProjectImport: {
                        ...
                        submenu : {
                            mnuProjectImportFile: {
                                label   : 'Из файла',
                                icon    : 'img/32/computer.png',
                                action  : project.import
                            },
                            mnuProjectImportRepo: {
                                label   : 'Из репозитория',
                                icon    : 'img/32/web_disk.png',
