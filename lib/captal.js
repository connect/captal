...
$(document).ready(function(){
    ...
    block.add = function(o) { debug('block.add(',JSON.stringify(o),')');        
        ...
        editor.on('blur',function(e){ debug('editor.blur()');
        ...
        editor.on('focus',function(e, ui){
            ...
            $('#nav').jstree(true).select_node( 'nav_'+parID+'_'+blkID , true);
        });
    ...
    block.create = function(o){ debug('block.create('+o.tag+')');
        ...
        var blkTypeN  = $('#desk .ui-tabs-panel[aria-hidden="false"] #blk_'+ o.tag +' .ace_editor').length + 1;
        ...
        tree.create_node( 'nav-'+ o.tag , { type: o.tag, id: 'nav_'+ o.paragraph +'_'+ blkID, text: blkTypeN });
    ...
    block.store = function(editorID){ debug('block.store('+ editorID +')');
        ...
        var index       = $('#tab-'+parID+' .blk_editor').index(obj);
        var tag         = obj.attr('captal-tag');
        var fields      = obj.prev();
        ...
        if (tag == 'handler') {
            //debug( name );
            $('#data-source handlers [paragraph='+ parID +'][id='+id+']')
                .html( '&lt;![CDATA[' + source + ']]&gt;' );
        } else {
    ...
    paragraph.create = function(o){ debug('paragraph.create('+ JSON.stringify(o) +')');
        ...
        if ($('#data-source').html().length > 0) {
            ...
            tree.create_node( o.parent.id , { 
                ...
                // update data-source
                $('#data-source folder'+parentDOM).append('<article id="'+newID+'"><text></text></article>');
    ...
    paragraph.show = function(parID) { debug('paragraph.show('+parID+')');
        ...
        $('#nav').jstree({
            ...
            if ( data.action == 'select_node' && (data.node.type == 'text' || data.node.type == 'action' || data.node.type == 'script' || data.node.type == 'handler') ) {                  
                ...
                if ( $('.ace_focus').attr('captal-name') != data.node.text) {
                    ...
                    block.show( paragraph.current(), data.node.id.split('_')[2] );
                }
            }
        ...
        $('#tab-'+ parID +' .blk_editor').each(function(index){           
            ...
            var nav = tree.create_node( 'nav-'+ tag , { type: tag, id: 'nav_'+ parID +'_'+ index , text: counts[tag] });
