...
$(document).ready(function(){
    ...
    block.add = function(o) { 
        ...
        if (o.tag == 'handler') {
            ...
            if (o.lang == 'undefined') {
                ...
                //lang.detect( o.body , function(stats){
                    ...
                    //$('#desk #tab-'+ o.id +' '+ o.tag +'[id="'+ o.name +'"] img.lang-flag').attr('src', 'src="img/16/flag_'+ o.lang +'.png"')                    
                    //'<img class="lang-flag" ><div class="blk_header" onclick="block.headerClick(this)"></span>'+o.name+':</div>'
                    //$('#data-source folder article#'+ o.paragraph +' '+ o.tag +'[id="'+ o.name  +'"]' ).attr('lang', o.lang);
                    /*
                    $('#data-source article#'+ o.paragraph +' '+ o.tag +'[id="'+ o.name  +'"]' )
                        .attr('lang', lang.detect(o.body, function(){
                            $('#data-source article#'+ o.paragraph +' '+ o.tag +'[id="'+ o.name  +'"]' ).attr()
                        }));

                    $('#data-source article#'+ o.paragraph +' '+ o.tag +'[id="'+ o.name  +'"]' ).attr('lang', o.lang);
                    */
                //});                
            }      
        ...
        if (o.tag == 'text') {
            ...
            if (o.translated === 'true') {
                ...
                var tran = $('#data-source translation[type="auto"][textid="'+o.name+'"]');
                var orig = $('#data-source translation[type="original"][textid="'+o.name+'"]');
                var tranText = tran
                                .html()
                                .replace( /\<\!\[CDATA\[/i,'')
                                .replace( /&lt;\!\[CDATA\[/i,'')
    ...
    block.create = function(o){ 
        ...
        if (o.tag == 'handler') {
            // if handler
            //debug(paragraph.id);
            $('#data-source handlers').append('<'+o.tag+' name="'+paragraph.id+'" id="'+o.name+'"></'+o.tag+'>');
        } else {
            // if article
            $('#data-source article#'+ paragraph.id).append('<'+o.tag+' goto="'+o.logicGO+'" id="'+o.name+'" if="'+o.logicTRUE+'" ifnot="'+o.logicFALSE+'" do="'+o.logicDO+'" isglobal="'+o.logicGLOBAL+'">&lt;![CDATA['+o.body+']]&gt;</'+o.tag+'>');            
        }                      
    ...
    /* function(obj) {
        ...
        if (confirm('Удалить блок?') ) {
            ...
            $(id).remove();
            $('#data-source article#'+ paragraph.id +' '+tag+'[id="'+ type +'"]').remove();
            $("#desk").tabs("refresh");

            // if it was last block create text
            if ( $('#data-source article#'+paragraph.id).html().trim() == '' ) {
                $('#data-source article#'+paragraph.id).append('<text id="1"></text>');
                paragraph.open(paragraph.id);
            }            
        }
    ...
    block.rename = function(e){                        
        ...
        if (newName != null) {
            ...
            if (tag == 'handler') {      
                ...
                $('#data-source handler[name='+paragraph.id+'][id="'+ oldName +'"]').attr('type', newName);
            } else {
                $('#data-source article#'+paragraph.id+' '+tag+'[id="'+ oldName +'"]').attr('type', newName);
            }
        }
    };
    ...
    block.store = function(o){ //debug('block.store('+blockID+')');
        ...
        if (tag == 'handler') {
            //debug( name );
            $('#data-source handlers [name='+ paragraph.id +'][id='+name+']')
                .html( '&lt;![CDATA[' + source + ']]&gt;' );
        } else {
            $('#data-source article#'+ paragraph.id +' '+tag +'[id="'+name+'"]')
                .html( '&lt;![CDATA[' + source + ']]&gt;' )
                .attr('if',         logicTRUE)
        ...
        lang.detect(source, function(res){
            ...
            if (tag != 'handler') {
                $('#data-source article#'+paragraph.id+' '+ tag +'[id="'+ name +'"]').attr('lang',tlang);
            }
        });
    };   
    ...
    block.translate = function(blockID){ debug('block.translate()');
        ...
        var text = $('#data-source text[id="'+ $('#'+blockID).attr('captal-name') +'"]')
                                                    .html()
                                                    .replace( /\<\!\[CDATA\[/i,'')
                                                    .replace( /&lt;\!\[CDATA\[/i,'')
                                                    .replace(']]>','')
        ...
        if ( $('#data-source article#'+parID).length == 0 && $('#data-source handler[name="'+parID+'"]').length == 0 ) {
            ...
            if (max > 0) {                               
                ...
                $('#data-source article#'+parID).children().each(function(){
                    ...
                    if ( $(this)[0].localName == 'metadata' || $(this)[0].localName == 'translation' || $(this)[0].localName == 'lastsave') { 
                        ...
                        block.add({
                            ...
                            body        : $(this)[0].innerHTML
                                                    ...
                                                    .replace(']]>','')
                                                    .replace(']]&gt;',''),
                            name        : $(this).attr('id')        || randomName, // attr.type
                ...
                */
                ...
                if ($('#data-source folder').length == 0) {
                    ...
                    if (last > 0) {                        
                        ...
                        $('#data-source book').append('<folder id="folder_0000" id="autofolder"><article id="0"><text id="text_0000"></text></article></folder>');
