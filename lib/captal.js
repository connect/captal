 ...
 //console.log(blkID,blkTypeN)       
    ...
    paragraph.setLabel = function(parID, value){        
        ...
        var parID   = (typeof parID == 'string') ? parID : undefined || paragraph.current(),
            ...
            value   = value || prompt('Введите ярлык для этого параграфа', current),
            nav     = $('#nav').jstree(true);
    ...
    paragraph.show = function(parID) { debug('paragraph.show('+parID+')');
        ...
        tree.set_text('nav-root', (label) ? parID+':'+label  : parID );
        ...
        $('#tab-'+ parID +' .blk_wrapper').each(function(index){           
            ...
            if (tag == 'script') {
                ...
                var name  = $(this).find('#logicGO').val();
        ...
        $('#data-source article:has(action[goto="'+parID+'"])').each(function(index){
            ...
            tree.create_node('nav-links', { type: 'link', id: 'nav֍'+parID+'֍link֍'+name, text: '§'+ (label) ? name+':'+label  : name });
        });
                ...
                */    
                ...
                if (data.node.id == 'toc-book') {
                    ...
                    if ( $('#data-source article#'+data.text).length == 0 ) {
                        ...
                        var label = $('#data-source article#'+data.text).attr('label') || false;
                        nav.set_text('nav-root', (label) ? data.text+':'+label  : data.text );
                        ...
                        if ( (!isNaN(data.old)  && data.old  > firstNumPar && data.text < firstNumPar) ||
                             (!isNaN(data.text) && data.text < firstNumPar) ) {
                            
                            // now this paragraph is first numeric, update old numeric one
                            paragraph.state( firstNumPar, 'refresh');
                            // update self state
                            paragraph.state( data.text , 'refresh' );
                        
                        } else if ( data.old == firstNumPar) {
