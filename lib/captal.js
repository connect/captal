...
$(document).ready(function(){
    ...
    getMarks = function(text, callback){ debug('getMarks()');                
        ...
        project.iCallback= callback;
        project.iText    = text;
        project.iNumbers = numbers;
        project.iNumber  = 0; // first

        getMarksProcess();
        setInterval(function(){
            
            $('#dlgLoading').html( project.iNumber  +'/'+ project.iNumbers.length );
        },2000);
    };
    
    // not to freeze browser we will process in chunks
    getMarksProcess = function() {
        ...
        for (var n = 0; n < processBy; n++ ) {

            if (project.iNumber + n == project.iNumbers.length ) {
                
                break;
            }

            var num = project.iNumbers[project.iNumber +n];
            
            // find first position of this number
            var pos = project.iText.indexOf(num);                         

            // for each position of this number in project.iText
            while( pos != -1) {

                // skip numbers in the very begining of project.iText
                if ( pos > 0 ) {

                    // check previous and next characters
                    prevCh = project.iText.charCodeAt(pos - 1);
                    nextCh = project.iText.charCodeAt(pos + num.toString().length);

                    // confirm they're not a number characters
                    if ( (prevCh < 48 || prevCh > 57 || isNaN(prevCh)) && (nextCh < 48 || nextCh > 57 || isNaN(nextCh) ) ) {

                        // cut left side of project.iText                                                    
                        left = project.iText.substring(0,pos).split(' ');
                        ...
                        // cut right side
                        right = project.iText.substring(pos + num.toString().length).split(' ');
                ...
                pos = project.iText.indexOf(num,pos+1); 
            }
        }
        ...
        if (num == project.iNumbers[ project.iNumbers.length-1 ]) {
            
            project.iNumber     = null;
            project.iNumbers    = null;            
            project.iText       = null;
            ...
            // return results
            if (typeof project.iCallback === 'function') {

                project.iCallback(marks);

            } else {       

                /*
                for (var t in marks){
                    console.log('"'+marks[t].prefix +'XXX'+ marks[t].affix+ '"', ':', marks[t].freq);
                }*/
 ...
 //console.log(blkID,blkTypeN)       
    ...
    paragraph.paste = function(){
        ...
        var parID     = paragraph.newID();
        var newPar    = $(sourcePar)
                            .attr('id', parID)
    ...
    paragraph.show = function(parID) { debug('paragraph.show('+parID+')');
        ...
        $('#nav').jstree({
            ...
            if ( data.action == 'select_node' ) { 
                ...
                if (data.node.type == 'text' || data.node.type == 'action' || data.node.type == 'script' || data.node.type == 'handler') {
                    ...
                    paragraph.open({ paragraphID: data.node.text.replace('§','') });
                }
            }
        ...
        $('#tab-'+ parID +' .blk_wrapper').each(function(index){           
            ...
            tree.create_node( 'nav-'+ tag , { type: tag, id: 'nav֍'+ parID +'֍'+ tag +'֍'+ (counts[tag]-1) , text: name });
        ...
        $('#data-source article:has(action[goto="'+parID+'"])').each(function(index){
            ...
            tree.create_node('nav-links', { type: 'link', id: 'nav֍link֍'+name, text: '§'+ name });
        });
        ...
        $('#data-source article').sort(alphanum).each(function(){ // alphanum.js
            ...
            $(this).children().each(function(){
                ...
                */
                ...
                if (tag == 'text' && tLang == sLang) {
                ...
                } else if (tag == 'action' && tLang == sLang) {
                    ...
                    doc.append('<blockquote>'+ body +' ('+logicGO+')</blockquote>');
                }
...
//$('#data-source').html(res);return;
    ...
    project.loadTXT = function(mode){ 
        ...
        var xml         = document.createElement('xml'),
            ...
            actMarkers  = $('#dlgImportTXT input[type="checkbox"]:checked'),
            line,
            trimLine,
            numLine,
            actHTML     = '',
        ...
        if (parMarker.attr('marker') == 'A') { // just a number
            
            res = res.split('\n');
            
            // create first paragraph to push intro
            ...
            for (var i in res){  // each import-source line
                ...
                if (trimLine == numLine) {                                                                                
                    ...
                    for (var a = 0; a <= actMarkers.length-1; a++) {
                        ...
                        var amark   = project.marks[ $(actMarkers[a]).attr('marker') ];                            
                        var abuffer = jbuffer.split( amark.prefix );

                        for (var j in abuffer) {

                            if (j > 0 && abuffer[j].indexOf(amark.affix) > 0 ){
                                // action found
                                var tact = abuffer[j].substring(1, abuffer[j].indexOf(amark.affix) );
                    ...
                    $(xml).find('article#'+parID).append('<text>&lt;![CDATA['+jbuffer+']]&gt;</text>\n'+actHTML+'\n');
                    buffer  = [];
                    actHTML = '';
            ...
            mark = project.marks[parMarker.attr('marker')];
            ...
            for (var i in res){  // each import-source line
                ...
                if ( trimLine.indexOf(mark.prefix) == 0 ) { // possible paragraph opening
                    ...
                    if ( mark.affix == '' || trimLine.indexOf(mark.affix) > 0 ) { 
                            ...
                            for (var j in abuffer) {
                                ...
                                if ( j > 0 ) { 
                                    ...
                                    if (amark.affix == '') {
                                        ...
                                        // cleanup
                                        if (tact.match(/[a-zA-Z0-9]+/) == null)
                                        console.log('->',tact,abuffer[j])
                        ...
                        if (trimLine.indexOf(mark.affix) > 0) {
                            // paired marker                            
                        ...
                        // false alarm, not paragraph declaration
                        buffer.push(line);
                    }
    ...
    project.loadXML = function(res, callback) { debug('project.LoadXML(...)');                
        ...
        if (book.length == 0){
            ...
            $('#toc').jstree({
                ...
                if ( data.action == 'select_node' && (data.node.type.indexOf('paragraph_') == 0 || data.node.type == 'handler') ) {  
                ...
                */    
                ...
                if (data.node.id == 'toc-book') {
                    ...
                    if ( $('#data-source article#'+data.text).length == 0 ) {
                        ...
                        tree.set_text(data.node, data.text, false);
                        ...
                        .each(function(index, value) {
                            ...
                            if (node.id.indexOf('nav֍'+data.old+'֍') == 0) {
                                
                                var newID = node.id.replace('nav֍'+data.old,'nav֍'+data.text);
...
//debug(page.pageIndex,tpage);
    ...
    project.preloadTXTold = function(raw){ debug('project.preloadTXT()'); 
        ...
        //console.log('markers',iParLeft,iParRight)
        ...
        for (var i in res) {
            ...
            // If we have paragraph markers
            if (cParTags && num != null) {                   
                
                // if line begins with paragraph opening marker
                if ( self.indexOf(iParLeft) == 0 ) {                            
    ...
    project.preloadTXT = function(raw){
        ...
        getMarks( raw.replace(/\n/g, ' ').replace(/\r/g,''), function(marks){
            ...
            for (var i in marks){     
                ...
                //console.log( '[',marks[i].prefix+'|'+marks[i].affix+']:'+marks[i].freq )
                
                $('#dlgImportTXT #pnlMarkOptions').append('<div class="automarker">\
                    <input type="radio" name="parMarker" marker="'+i+'"> <input type="checkbox" marker="'+i+'"> '+marks[i].prefix+'<span>xxx</span>'+marks[i].affix+' ('+marks[i].freq+')\
                </div>');
            }
    ...
    gui.createFolder = function(){
        ...
        if ($('#data-source').html().length > 0) {
            ...
            tree.create_node('toc-paragraphs',{
                ...
                // update data-source
                $('#data-source folder:last').before('<folder id="'+node.id+'" name="New folder" id="folder_yellow"></folder>');
            });
        } else {
            error('Проект не загружен');
        }
