...
$(document).ready(function(){
     ...
     * @returns {String}
     */ 
    getMarks = function(text, callback){ debug.log('getMarks()');                
            ...
            for (var j in abuffer) {
                ...
                if ( j > 0 ) {
                    ...
                    if (tact != '' && tact != null && actHTML.indexOf('goto="'+tact+'"') == -1) {
                        ...
                        actIndex++;
                        actHTML += '<action goto="'+tact+'" intext="'+actIndex+'">&lt;![CDATA['+ tact +']]&gt;</action>\n';
                    }
                }
            }                                                        
        }
    ...
    lang.freq = {
        ...
        rus: ['артефакт','без','большой','бы','быть','в','вам','вас','ваш','вверх','вернитесь','весь','вниз','во',
            ...
            'ты','у','уже','хотеть','человек','что','что','чтобы','это','этот','юг','я','хотя','думаю','подождите','подождете'],
        ...
        eng: ['the','be','of','and','in','to','have','it','for','I','that','you','he','on','with','do','at','by',
            ...
            'their','go','what','there','all','get','her','make','who','out','rather','up','see','know','time','take','them',
    ...
    lang.build = function(l){
        ...
        for (var i in def){
            ...
            if (dict[i] == undefined) {
                ...
                debug.log('No locale string for: '+i);
                dict[i] = '!'+ def[i]; // get missing lines from default dictionary
            }
        }
    };
    
    lang.detectYan = function(str, callback){ debug.log('lang.detect(...)');
     ...
     */
    lang.detect = function(str, callback){ debug.log('lang.detect(...)');
     ...
     */
    block.add = function(o) { debug.log('block.add(...)');        
        ...
        var tmpl    = $('#block-wrapper-template')
                        ...
                        .replace(/{IF_INTEXT_READONLY}/ig, o.intext != '' ? 'disabled' : '');        
        ...
        editor.on('change',function(e,ui){ // resize translation metablocks
            ...
            if (e.data != undefined && $(ui.container).attr('captal-tag') == 'text'){
                ...
                /*if (e.data.action == 'insertText') {
                    ...
                    if ( e.data.text == 'Х' ){
                        ...
                        if (right.match(/Х([^{}]*)Ъ/g)) {

                            right = right.replace(/Х([^{}]*)Ъ/g, function(item){

                                return '{'+ (item.substring(1, item.length-1)) +'}';
                            });

                            body = left.substr(0, left.length) + right;
                            editor.setValue(body.toString());
                            editor.resize(true);
                            editor.clearSelection();
                            editor.moveCursorTo(curPos.row, curPos.column );

                            return false;
                        }
                        
                    } else if ( e.data.text == 'Ъ' ) {
                    
                        left += 'Ъ';
                        
                        if (left.match(/Х([^{}]*)Ъ/g)) {

                            left = left.replace(/Х([^{}]*)Ъ/g, function(item){

                                return '{'+ (item.substring(1, item.length-1)) +'}';
                            });

                            body = left + right.substr(1);
                            editor.setValue(body.toString());
                            editor.resize(true);
                            editor.clearSelection();
                            editor.moveCursorTo(curPos.row, curPos.column );
                ...
                */
                ...
                if (e.data.action == 'insertText' && (e.data.text == '#' || e.data.text == '№') ){
                    ...
                    if ((left.lastIndexOf('#') == left.length-1 || left.lastIndexOf('№') == left.length-1 ) && left.length > 0) {
                        ...
                        /*
                        body = left.substr(0, left.length) +'#'+ right.substr(1);
                        editor.setValue(body.toString());
                        editor.resize(true);
                        editor.clearSelection();
                        editor.moveCursorTo(curPos.row, curPos.column);
        ...
        editor.on('blur',function(e){ 
            ...
            debug.log('editor.blur('+editorID+')');
        ...
        editor.on('focus',function(e, ui){
            ...
            debug.log('focus gained:'+id+', '+parID+', '+blkID);            
        ...
        if (o.intext == undefined || o.intext == '' || o.tag != 'action') {
    ...
    block.copy = function(){ 
        ...
        debug.log('block.copy('+parID+', '+tag+', '+blkID+')');
     ...
     */
    block.create = function(o, callback){ debug.log('block.create('+o.tag+')');
     ...
     * @param {} obj
     */
    block.delete = function(o){ debug.log('block.delete()');
    ...
    block.move = function(e){ debug.log('block.move('+e+')');
    ...
    block.parseActions = function(){
        ...
        // if no editors are opened yet, retry later
        if ( block.current() == undefined) {
            
            setTimeout(block.parseActions, 500);
            return;
        }
        
        // try to prevent simultanious execution
        if (block.busyParsingActions) return;
        
        block.busyParsingActions = true;
        
        var parID    = paragraph.current(), 
            editorID = block.current(),
            //blkID    = $('#tab-'+parID+' #blk_text .ace_editor').index( $('#'+ editorID) ), 
            editor   = ace.edit(editorID),
            body     = editor.getValue().replace(/№\d+\b/g, function(item){ return '#'+ item.substr(1); }),
        ...
        for (var i in acts){
            ...
            if (actDOM.length == 0) {
                ...
                if ( actDOM.html().replace('&lt;![CDATA[','').replace(']]&gt;','') != actText ) {
                    ...
                    // udpade intext of editor
                    //$('#tab-'+parID+' #blk_action .ace_editor[captal-intext][captal-goto="'+tact+'"]').attr('captal-intext', c);
        ...
        if ( sorted.last().attr('intext') != unsorted.last().attr('intext') ) {    
            ...
            // update navigator
            gui.nav.build(parID, function(){
                
                delete block.busyParsingActions;
            });
        ...
        if (cfg.guiHideCoding){
            ...
            $('#tab-'+parID+' #blk_action .blk_wrapper:has([captal-intext])').hide();
            
            // hide empty group
            if ( $('#tab-'+parID+' #blk_action .blk_wrapper:has([captal-intext])').length == $('#tab-'+parID+' #blk_action .blk_wrapper').length ) {
                
                $('#tab-'+parID+' #blk_action').hide();
            } else {
                
                $('#tab-'+parID+' #blk_action').show();
            }
        }
    };
    ...
    block.rename = function(e){
        ...
        if (newName != null) {
            ...
            // update data-source
            if (tag == 'handler') {      
                //debug.log('handler[name='+paragraph.id+']')
     ...
     * Scroll to block
     */
    block.show = function(parID, tag, blkID) { debug.log('block.show('+parID+', '+tag+', '+blkID+')');
    ...
    block.spellcheck = function(editorID){ debug.log('block.spellcheck()');
     ...
     */
    block.store = function(editorID){ debug.log('block.store('+ editorID +')');
    ...
    block.translate = function(editorID, parID, blkID){ debug.log('block.translate('+editorID+', '+parID+', '+blkID+')');
    ...
    block.translated = function(o){ debug.log('block.translated(...)');                
    ...
    debug.revert = function(parID, index){
        
        $('#dlgDebug #path .item:gt('+index+')').remove();
        $('#dlgDebug #path .item:eq('+index+')').remove();
    ...
    paragraph.addTag = function(){ debug.log('paragraph.addTag()');
    ...
    paragraph.close = function(obj, callback){ debug.log('paragraph.close(...)');
    ...
    paragraph.copy = function(obj){ 
        ...
        debug.log('paragraph.copy('+parID+')');
    ...
    paragraph.create = function(o){ debug.log('paragraph.create(..)');
    ...
    paragraph.delete = function(parID, noconfirm){ debug.log('paragraph.delete('+parID+')');
    ...
    paragraph.delTag = function(){ debug.log('paragraph.delTag()');
    ...
    paragraph.move = function(target){ debug.log('paragraph.move('+target+')');
    ...
    paragraph.new = function(o){ debug.log('btnNewParagraph.click()');

        var parID = (typeof o == 'object') ? o.paragraphID : o;
        
        o = (typeof o != 'object') ? {} : o;
        
        // check if paragraph exist
        if ( $('#data-source article#'+parID).length == 0 || o.ignore ){
                ...
                if (tnode.type.indexOf('folder') == 0) {            
                    ...
                    if ( $('#toc .jstree-last[toc-role="group"] li').length < paragraph.groupBy ) {                
                        ...
                        tree.create_node( 'toc-paragraphs', {
                                ...
                                state: { opened : true },
                                text: '..',
                                li_attr: { 'toc-role':'group', 'captal-type': 'autofolder' }
                            },
                            'last',
                            function(tparent){                        
                                paragraph.create({
    ...
    paragraph.newInject = function(){ debug.log('paragraph.newInject()');
    ...
    paragraph.open = function(o){
        ...
        debug.log('paragraph.open('+ parID +','+ curParID +')');
    ...
    paragraph.paste = function(){
        ...
        if ( $('#copy-buffer').attr('type') == 'paragraph' ) {
            ...
            paragraph.new({ paragraphID: parID, ignore: true }); // ignore par exist
        }
    };
    
    paragraph.rename = function(parID, value){ debug.log('paragraph.name('+parID+','+value+')');
        ...
        if (parID && value) {
                ...
                if (node.id == 'toc-book') {
                    ...
                    if ( $('#data-source article#'+value).length > 0 ) {
                        ...
                        var tRegEx =  new RegExp('\#'+parID+'\\b', 'g');
                        
                        
                        // update intext links
                        //
                        var intextArr = $('#data-source text')
                                .filter(function(){ return $(this).html().match( tRegEx ) });
                        
                        $(intextArr).each(function(index){
                            
                            var parID    = $(this).parent().attr('id'),
                                blkID    = $('#data-source article#'+parID+' text').index( $(this) ),
                                //editorID = $('#tab-'+parID+' #blk_text .ace_editor:eq('+blkID+')').attr('id'),
                                //editor   = ace.edit(editorID),
                                newBody  = $(this).html().replace(tRegEx, '#'+value);
                            
                            $(this).html( newBody );
                            
                            //paragraph.close(parID);
                            
                            //editor.setValue( newBody, 0 );
                            //editor.resize(true);
                        });
    ...
    paragraph.show = function(parID) { debug.log('paragraph.show('+parID+')');
        ...
        if (cfg.guiHideCoding){
            ...
            gui.hideCoding(parID);
           
        } else {
            
            gui.showCoding(parID);
        }
    };    
    
    // shuffle all numeric paragraphs, preserve position in user folders
    paragraph.shuffle = function(){ debug.log('paragraph.shuffle()');
        ...
        if ($('#data-source article').length > 1) {
            ...
            // get list of numereic paragraph id's and shuffle them
            var res = $('#data-source article')
                .filter(function(){                    
                    //return $(this).attr('id').match(/\d+/);
                    return !isNaN(parseInt( $(this).attr('id') ));
                })
                .sort(alphanum) // alphanum.js
                .map(function(){ 
                    return $(this).attr('id');
                })
                .get();
    ...
    paragraph.state = function(parID, state){ debug.log('paragraph.state()');
    ...
    project.addComponent = function(o){ debug.log('project.addComponent(',o);
    ...
    project.export = function(){ debug.log('project.export()');
    ...
    project.exportDOC = function(mode){ debug.log('project.exportDOCX()');
        ...
        $('#data-source article').sort(alphanum).each(function(){ // alphanum.js
            ...
            $(this).children().each(function(){
                ...
                } else if (tag == 'action' && cExportActions && ( tLang == sLang || sLang == 'all') && intext == '' ) {
                        
                    var link = '<a class="MsoHyperlink" href="#'+logicGO+'"><span class="CaptalLink">'+ logicGO +'</span></a>',
                        expr = new RegExp( logicGO +'\\b', 'g');
                    
                    
                    
                    if ( body.match(expr) == null ) {
                        
                        body += '  ('+ link +')' ;
                        
                    } else {
                        
                        body = body.replace( expr, link);
                    }
                                        
                    body = '<p class="CaptalAction">'+ body +'</p>';
                ...
                if (art == artMax && blk == blkMax) { // last
                    ...
                    if (mode == 'preview') {
                        ...
                        if (cfg.lineBreak) {
                            ...
                            doc.prepend('<style>\n\
                                            p.CaptalParagraph{ white-space: pre; }\n\
                                        </style>');
                        }
    ...
    project.importDOCX = function(f){ debug.log('project.importDOCX()');
        ...
        jf.read().then(function(res){ 
            ...
            debug.log('Reading error', e);
        });                                
        
    };
    
    project.importPDF = function(f){ debug.log('project.importPDF()');
    ...
    project.importTXT = function(f, encoding){ debug.log('project.importTXT()');
    ...
    project.loadTXT = function(mode){ 
        ...
        mode = mode || 'import';        
        debug.log('project.loadTXT('+mode+')');
     ...
     * @returns {undefined}
     */
    project.loadXML = function(res, callback) { debug.log('project.LoadXML(...)');                
        ...
        setTimeout(function(){
            ...
            if (book.length == 0){
                ...
                .on("ready.jstree",function(e, data){ debug.log('toc.jstree.ready()');
                     ...
                     * @returns {undefined}
                     */
                    var loadArticles = function() { debug.log('load articles()');
                ...
                .bind('move_node.jstree', function(e,data){ debug.log('move_node.jstree()');
                    ...
                    if (oldParent != newParent){
                        ...
                        //                                 
                        debug.log('node',data.node.id,'changed folder from',oldParent,'to',newParent);
                        ...
                        debug.log('node',data.node.id,'moved to',data.position);
    ...
    project.options = function(opt){ debug.log('project.options('+opt+')');
        ...
        $('#dlgOptions').dialog({
            ...
            create: function( event, ui ) {
                ...
                $('#dlgOptions #cGUIHideCoding').prop('checked', opt == 'wizard' ? true  : cfg.guiHideCoding);
    ...
    project.preloadDOCX = function(doc){ debug.log('project.preloadDOCX()');        
    ...
    project.preloadPDF = function(pdf){ debug.log('project.preloadPDF()');
    ...
    project.rename = function(newName){ debug.log('project.rename('+newName+')');
    ...
    project.run = function(){ debug.log('project.run()');
            ...
            $('#dlgDebug').dialog({
                ...
                create: function(){
                    ...
                    jsIQ.addEventHandler('before_render_article', function(params, vars, process){
                        ...
                        params.title = params.article.title;
                        ...
                        var html,
                            ...
                            tsearch,
                            parID    = params.title,
                            index    = $('#dlgDebug #path .item').length,
                            label    = $('#data-source article#'+parID).attr('label') || '',
                        ...
                        // append navigation
                        $('#dlgDebug #path')
                        .append('<div class="item" onclick="debug.revert('+parID+','+index+')">'+
                                    '<div>'+  index +'</div>'+
                                    '<div>§'+ parID +'</div>'+
                                    '<div>'+  label +'</div>'+
                                '</div>');
    ...
    repo.game.get = function( gameID ){
        debug.log('repo.game.get('+ gameID +')');                
    ...
    repo.component.get = function( comID ){
        debug.log('repo.component.get(',comID,')');
    ...
    gui.createTabs = function(){ debug.log('gui.createTabs()');
        
        
        $('#desk').tabs({
            collapsible: false,
            activate: function( e, ui ) { 
                debug.log('tabs.activate(...)');
    ...
    gui.fullscreen = function () {
        debug.log('gui.fullscreen()');
        
        var element = document.getElementById('html');
        
        if (element.requestFullscreen) {
            debug.log('requestFullscreen')
            element.requestFullscreen();
        } else if(element.webkitrequestFullscreen) {
            debug.log('webkitrequestFullscreen')
            element.webkitRequestFullscreen();
        } else if(element.mozRequestFullscreen) {
            debug.log('mozRequestFullScreen')
    ...
    gui.hideCoding = function(parID){
        ...
        if (parID == undefined) {
            ...
            $('head style#captal-nocoding').html('\
    ...
    gui.logicFieldUpdate = function(e, doStore){
        ...
        debug.log('gui.logicFieldUpdate('+editorID+', field:'+field+', store:'+doStore+')') ;
    ...
    gui.logicGO = function(e){ debug.log('gui.logicGO()');
    ...
    gui.maximize = function(){ debug.log('gui.maximize()');                    
    ...
    gui.nav.build = function(parID, callblack){
        ...
        var nav     = $('#nav').jstree(true),
            lastBlk = $('#tab-'+ parID +' .blk_wrapper').length -1;
    ...
    gui.tagList = function(tagID){ debug.log('gui.tagList()');
    ...
    // refresh folder name
    gui.tocUpdate = function(parent){ debug.log('gui.tocUpdate('+parent+')');
        
        if (parent == undefined) return;
        
        var toc = $('#toc').jstree(true);
                        
        var node = (parent != undefined) ? toc.get_node(parent) : toc.get_node( toc.get_node(paragraph.current()).parent );
        var text = [0,0];
        
        // check folder type, don't rename user folders
        if (node.type != 'autofolder') return;
        
        // get list of autofolders
        var autofolders = $('#toc [captal-type="autofolder"]').get();
        ...
        if (text[0] == undefined && text[1] == undefined) {
            ...
            // no more nodes here, remove
            if (autofolders.length > 1) {
        
                toc.delete_node( node );
                
            } else {
            
                // but leave the last autofolder
                toc.set_text( node, lang.current.folder_auto );
            }
        } else {
        
            toc.set_text( node, text.join('..') );
        }
    };  
    ...
    gui.setFlag = function(res){ debug.log('gui.setFlag()',res);
    ...
    gui.setFlagYandex = function(res){ debug.log('gui.setFlag()',res);
    ...
    gui.showCoding = function(parID){
        ...
        $('#tab-'+parID+' #blk_action').show();
        $('#tab-'+parID+' #blk_action .blk_wrapper:has([captal-intext])').show();
    };
    ...
    gui.load(function(){
        ...
        $('#mnuAutolink').menu({
            select: function(e, ui){ debug.log('mnuAutolink()');
