...
$(document).ready(function(){
     ...
     */
    block.add = function(o) { 
        debug('block.add(',o.name,o.tag,o.lang,')');        
        ...
        if (o.tag == 'handler') {
            $('#desk #tab-'+ o.id).append(
                ...
                <div id="'+                 o.uid+'" \
                    ...
                    captal-do="" \
                    captal-isglobal="" \
                    class="blk_editor">'+   o.body +'</div></div>' );
        } else {
            if (o.lang == 'undefined') {
            ...
            $('#desk #tab-'+ o.id +' #blk_'+o.tag).append(
                '<div class="blk_wrapper"><img class="lang-flag" src="img/16/flag_'+o.lang+'.png" lang="'+ o.lang +'"><div class="blk_header" onclick="block.headerClick(this)"></span>'+o.name+':</div>\
                <div id="'+                 o.uid+'" \
                    captal-paragraph="'+    o.paragraph+'" \
                    captal-name="'+         o.name+'" \
                    captal-tag="'+          o.tag+'" \
     ...
     */
    block.create = function(o){ 
        debug('block.create('+o.tag+')');
        ...
        if (o.tag == 'handler') {
            ...
            // if article
            $('#data-source article#'+ paragraph.id).append('<'+o.tag+' goto="'+o.logicGO+'" type="'+o.name+'" if="'+o.logicTRUE+'" ifnot="'+o.logicFALSE+'" do="'+o.logicDO+'" isglobal="'+o.logicGLOBAL+'">&lt;![CDATA['+o.body+']]&gt;</'+o.tag+'>');            
        }                      
    ...
    block.translate = function(){
        
        var text = ace.edit(block.id).getValue().replace( new RegExp('\n', 'g'), '<br />');
        var recognizedLang = $('#'+block.id).prev().prev().attr('lang');
        var supportedLangs = {
            'rus'   : 'ru',
            'ukr'   : 'ua',
            'eng'   : 'en',
        ...
        $.ajax({
            ...
            data: {
                ...
                format  : 'html',
                options : 0,
                callback: 'block.translated'
            }
        });
    };
    
    block.translated = function(res){
        ...
        var sourceLang      = supportedLangs[lang[0]] || 'eng';
        var targetLang      = supportedLangs[lang[1]] || 'rus';
        ...
        $('#'+block.id)
            ...
            .after('<div class="autotrans"><b>Автоперевод: <img src="img/16/flag_'+targetLang+'.png"></b><div>'+ res.text[0] +'</div>Переведено сервисом <a href="http://translate.yandex.ru/" target="_blank">«Яндекс.Переводчик»</a></div>\
                <div class="origtrans"><b>Оригинал: <img src="img/16/flag_'+sourceLang+'.png"></b><div>'+editor.getValue()+'</div></div>');
        
        editor.focus();
        editor.setValue('').resize(true);
        //$('#'+block.id).trigger('click');
        //block.load( block.id );
    };
    ...
    paragraph.open = function(parID, callback){        
...
//$('#data-source').html(res);return;
            ...
            $('#toc').jstree({
                ...
                */
                ...
                if (data.node.id == 'toc-book') {
                    ...
                    $('title').html( data.text + ' - Каптал ' + $('.version').html() );
                ...
                if ($('#data-source options').length == 0){
                    ...
                    $('#data-source book').append('<options><set id="lineBreak">'+cfg.lineBreak+'</set></options>');
                    
                } else {
                    
                    cfg.lineBreak = $('#data-source options #lineBreak').val();
                }
    ...
    project.new = function(opt, projName){
        ...
        cfg.readonly    = 0;
        project.gameID  = null;
        projName        = projName || 'Безымянный';
        
        $('#new-project-template #book').attr('title', projName);
    ...
    project.options = function(opt){
        ...
        if (opt == 'apply') {
            ...
            //project.name = $('#dlgOptions #projName').val();            
            
            $('#new-project-template #lineBreak').html(cfg.lineBreak == true ? 1 : 0);
        }
...
//debug(page.pageIndex,tpage);
        ...
        $('#toolbar #logicTRUEicon').click(function(e){
            ...
            if ( fn != '' ) {
                ...
                if ($('#desk article#'+paragraph.id+' script[captal-name="'+ fn +'"]').length != 0) {
                    ...
                    if ($('#data-source script[type="'+ fn +'"]').length != 0) {
                        // found, get paragraph
                        var parID = $('#data-source script[type="'+ fn +'"]').parent().attr('id');
    ...
    gui.maximize = function(){ debug('gui.maximize()');                    
        ...
        if (cfg.maximized) {                        
            ...
            // resize then show
            $('#desk').toggleClass('maximized',500,'easeOutBounce', function(){
            ...
            $('#table-of-contents').toggleClass('maximized');
            $('#navigator').toggleClass('maximized');
            $('#desk').toggleClass('maximized',500,'easeOutBounce');                                    
