...
$(document).ready(function(){
                ...
                for (var j in res) {
                    ...
                    for (var i in project.imp.marks){
                        ...
                        if (project.imp.marks[i].prefix == res[j].prefix && project.imp.marks[i].affix == res[j].affix) {
                            ...
                            project.imp.marks[i].freq = res[j].freq;
                            found = true;
                        }
                    }
    ...
    processActions = function(text, actMarkers, cActString, cActRemove){                
        ...
        for (var a = 0; a <= actMarkers.length-1; a++) {
            ...
            var amark   = project.marks[ actMarkers[a] ]; 
    ...
    block.addLinksNEW = function(){ debug('block.addLinks()');
        ...
        for (var i in ulinks){
            ...
            caret   = text.indexOf(link,caret+1); // move caret                
            text    = text.replace(new RegExp(link,'g'), function(p0,p1){
                
                var prevCh = text.charCodeAt(p1-1);                    
                var nextCh = text.charCodeAt(p1+link.length);
 ...
 //console.log(blkID,blkTypeN)       
    ...
    paragraph.show = function(parID) { debug('paragraph.show('+parID+')');
        ...
        $('#nav').jstree({
            ...
            if ( data.action == 'select_node' ) { 
                ...
                if (data.node.type == 'text' || data.node.type == 'action' || data.node.type == 'script' || data.node.type == 'handler') {
                    ...
                    if( data.node.type == 'action' && keys.state[18] && data.event.type == 'click') { // ALT+Click
        ...
        // navigator root is paragraph name or title
        ...
        $('#data-source article:has(action[goto="'+parID+'"])').each(function(index){
            ...
            tree.create_node('nav-links', { type: 'link', id: 'nav֍'+parID+'֍link֍'+name, text: '§'+ name });
        });
        ...
        $('#data-source article#'+parID+' metadata tag').each(function(){
            ...
            tree.create_node('nav-tags', { type: 'tag', id:'nav֍'+parID+'֍tag֍'+name, text: name });
        });
            ...
            $(this).children().each(function(){
                ...
                */
                ...
                if (tag == 'text' && ( tLang == sLang || sLang == 'all') ) {
                    ...
                    body += ( $.trim(body) != logicGO) ? ' ('+ logicGO +')' : '';
                    ...
                    doc.append('<a href="#'+logicGO+'">'+ body +'</a>');
                }
...
//$('#data-source').html(res);return;
    ...
    project.loadTXT = function(mode){ 
        ...
        // delete null paragraph from template
    ...
    project.preloadTXT = function(raw){
        ...
        getMarks( raw.replace(/\n/g, ' ').replace(/\r/g,''), function(marks){
            ...
            for (var i in marks){     
                ...
                marks[i].freq = (marks[i].freq == undefined) ? '*' : marks[i].freq; 
                
                $('#dlgImportTXT #pnlMarkOptions').append('<div class="automarker">\
                    <input type="radio" name="parMarker" marker="'+i+'" '+checked+'> <input type="checkbox" marker="'+i+'"> '+marks[i].prefix+'<span>xxx</span>'+marks[i].affix+' ('+marks[i].freq+')\
                </div>');
            }
