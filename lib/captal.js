...
$(document).ready(function(){
    ...
    block.add = function(o) { debug('block.add(...)');        
        ...
        $(editor.textInput.getElement()).on('keydown',function(e, ui){
            ...
            } else if ( $('.ace_spellcheck').length > 0  && ([32,37,38,39,40].indexOf(e.keyCode) >= 0 || ['!','?',':',',','.'].indexOf(e.key) >= 0)) {
                // if there are spellcheck errors, repeat checks
                //block.spellcheck();
                //block.spellcheck('local')
            }
    ...
    block.spellcheck = function(editorID){
