...
$(document).ready(function(){
        ...
        if (o.tag == 'handler') {
            ...
            $('#desk #tab-'+ o.id +' #blk_'+o.tag).append(
                ...
                <div id="'+                 o.uid+'" \
                    ...
                    captal-translated="'+   o.translated+'" \
                    class="blk_editor">'+ o.body +'</div></div>' );
            ...
            if (o.tag == 'text') {
                ...
                if (o.translated === 'true') {
                    ...
                    var tran = $('#data-source tran[target="'+o.name+'"]');
                    var orig = $('#data-source orig[target="'+o.name+'"]');
                    var tranText = tran
                                    .html()
                                    .replace( /\<\!\[CDATA\[/i,'')
                                    .replace( /&lt;\!\[CDATA\[/i,'')
    ...
    block.translate = function(blockID){ debug('block.translate()');
        ...
        var blockID = blockID || block.id;                                           
        var text = $('#data-source text[type="'+ $('#'+blockID).attr('captal-name') +'"]')
                                                    .html()
                                                    .replace( /\<\!\[CDATA\[/i,'')
                                                    .replace( /&lt;\!\[CDATA\[/i,'')
                                                    .replace(']]>','')
    ...
    block.translated = function(res){ debug('block.translated(...)');
        ...
        var obj         = $('#'+block.id);
        var blockName   = obj.attr('captal-name');
        ...
        var origText    = $('#data-source text[type="'+blockName+'"]')
                                    .html()
                                    .replace( /\<\!\[CDATA\[/i,'')
                                    .replace( /&lt;\!\[CDATA\[/i,'')
                                    .replace(']]>','')
        ...
        // update data-source
        // <article id="xx">
        //    <text type="text_0000" lang="undefined" if="" ifnot="" goto="" do="" isglobal="false">&lt;![CDATA[]]&gt;</text>
        //    <orig id="orig_0000" target="text_0000" lang="undefined">&lt;![CDATA[]]&gt;</orig>
        //    <tran id="tran_0000" target="text_0000" lang="undefined">&lt;![CDATA[]]&gt;</tran>
        
        $('#data-source text[type="'+blockName+'"]')
                .attr('translated', true)
                .after('<orig id="'+origID+'" target="'+blockName+'" lang="'+origLang+'">&lt;![CDATA['+origText+']]&gt;</orig>')
                .after('<tran id="'+tranID+'" target="'+blockName+'" lang="'+tranLang+'">&lt;![CDATA['+tranText+']]&gt;</tran>');
        
        if ( $('#'+block.id).hasClass('ace_editor') ) {            
        ...
        if ( $('#data-source article#'+parID).length == 0 && $('#data-source handler[name="'+parID+'"]').length == 0 ) {
            ...
            if (max > 0) {                               
                ...
                $('#data-source article#'+parID).children().each(function(){
                    ...
                    if ( $(this)[0].localName == 'metadata' || $(this)[0].localName == 'orig' || $(this)[0].localName == 'tran') { 
        ...
        for (var i in res) { // each data-source line
            ...
            if ( (cParTags && pProb == pProbMax) || (!cParTags && num == parseInt(tpar)+1 && num == line) || (cParUnsort && num == line) ) {
                ...
                if (buffer != []) {
                    ...
                    $(xml).find('article#'+tpar).append('<text type="'+getRandomName('text')+'" lang="'+lang.detect(jbuffer)+'">&lt;![CDATA['+jbuffer+']]&gt;</text>\n'+actText);
                    buffer  = [];
                    tbuffer = '';
                    actText = '';
                }
